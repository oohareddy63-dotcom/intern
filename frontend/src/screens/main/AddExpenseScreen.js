...
import { addExpense, clearError } from '../../store/slices/expenseSlice';
...
export default function AddExpenseScreen({ navigation }) {
  ...
  const handleAdd = async () => {
    ...
    if (result.error) showAlert('Error', result.payload || 'Failed to add expense');
    else { showAlert('Success', 'Expense added!'); navigation.navigate('Expenses'); }
  };
  ...
  return (
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* Submit */}
        <TouchableOpacity style={[s.btn, isLoading && s.btnOff]} onPress={handleAdd} disabled={isLoading}>
          {isLoading
            ? <ActivityIndicator color={COLORS.white} size="small" />
            : <><Icon name="add-circle" size={20} color={COLORS.white} style={{ marginRight: 8 }} /><Text style={s.btnText}>Add Expense</Text></>
          }
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  amountCard: {
    backgroundColor: COLORS.neonPurple, borderRadius: 20,
    padding: 28, alignItems: 'center', marginBottom: 16,
    shadowColor: COLORS.neonPurple, shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.4, shadowRadius: 20, elevation: 10,
  },
  amountLabel: { fontSize: 11, color: 'rgba(255,255,255,0.7)', fontWeight: '700', letterSpacing: 2, marginBottom: 8 },
  amountRow: { flexDirection: 'row', alignItems: 'center' },
  amountCurrency: { fontSize: 36, fontWeight: '700', color: 'rgba(255,255,255,0.7)', marginRight: 4 },
  amountInput: { fontSize: 52, fontWeight: '800', color: COLORS.white, minWidth: 120, textAlign: 'center' },
  section: {
